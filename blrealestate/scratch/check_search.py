import urllib.request, json, ssl
BASE = 'https://hisarpropertybazar.com/blapis/api'
token = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..XyJbkiSX-euVfj-FGYzDCVIuS5s6pWQRDiqs0tTZkYo'

ctx = ssl.create_default_context()
ctx.check_hostname = False
ctx.verify_mode = ssl.CERT_NONE

def req(path):
    r = urllib.request.Request(BASE + path, headers={'Authorization':f'Bearer {token}'})
    with urllib.request.urlopen(r, context=ctx, timeout=20) as res:
        return json.loads(res.read())

print("NO SEARCH:")
p1 = req('/ChannelPartner/properties?page=1&per_page=20')
print(f"Total: {p1.get('data', {}).get('total')}")

print("SEARCH=plot:")
p2 = req('/ChannelPartner/properties?page=1&per_page=20&search=plot')
print(f"Total: {p2.get('data', {}).get('total')}")

print("Q=plot:")
p3 = req('/ChannelPartner/properties?page=1&per_page=20&q=plot')
print(f"Total: {p3.get('data', {}).get('total')}")

print("KEYWORD=plot:")
p4 = req('/ChannelPartner/properties?page=1&per_page=20&keyword=plot')
print(f"Total: {p4.get('data', {}).get('total')}")
